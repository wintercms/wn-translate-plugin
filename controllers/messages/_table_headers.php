...
<script type="text/template" id="<?= $this->getId('fromTitle') ?>">
    <div class="header-language" onclick="$.translateMessages.toggleDropdown('#fromTitleButton')">
        <?php if ($selectedFrom === null): ?>
            <?= $defaultLocale->name ?> <span class="is-default">- <?= e(trans('winter.translate::lang.locale.default_suffix')) ?></span>
        <?php else: ?>
            <?= $selectedFrom->name ?>
        <?php endif ?>
...
<div class="dropdown dropdown-from">
    ...
    <ul class="dropdown-menu" role="menu" data-dropdown-title="Language">
        <?php if ($selectedFrom !== null && $selectedTo !== null): ?>
            <li role="presentation">
                <a
                    ...
                    <?= $defaultLocale->name ?> - <?= e(trans('winter.translate::lang.locale.default_suffix')) ?>
                </a>
            </li>
        <?php endif ?>
        ...
        <?php if (count($locales) <= 1): ?>
            <li class="no-other-languages text-muted"><small><?= e(trans('winter.translate::lang.locale.not_available_help')) ?></small></li>
        <?php endif ?>
    </ul>
</div>
...
<script type="text/template" id="<?= $this->getId('toTitle') ?>">
    ...
    <?php if ($selectedTo !== null): ?>
        <div class="header-hide-translated custom-checkbox">
            ...
            <label for="hideTranslated"><?= e(trans('winter.translate::lang.messages.hide_translated')) ?></label>
        </div>
    <?php endif ?>
</script>
...
<script type="text/template" id="<?= $this->getId('foundTitle') ?>">
    <?= e(trans('winter.translate::lang.messages.found_title')) ?> <i class="icon-info-circle" style="cursor: help" title="<?= e(trans('winter.translate::lang.messages.found_help')) ?>"></i>
</script>
